...
class AuthenticationService {
  ...
  public async authenticateUserOAuth(email: string, password: string, scope: string): Promise<AuthToken | null> {
   ...
   public async authenticateUserBasicAuth(email: string, password: string, scope: string): Promise<AuthToken | null | undefined> {
    ...
    if (result !== undefined) {
      if (result) {
        ...
        return null; // Indicate that authentication failed or no result is available
      }
    } else {
      return undefined; // Indicate that result is undefined
    }
  }
