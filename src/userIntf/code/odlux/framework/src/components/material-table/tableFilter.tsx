...
class EnhancedTableFilterComponent extends React.Component<IEnhancedTableFilterComponentProps> {
  ...
  createInputFilterHandler = (property: string) => (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    const filterValue = event.currentTarget.value.trim().split('*')[0];
    console.log(filterValue) 
    if(filterValue !== ''){
      const modifiedFilterValue = filterValue + '*';
      this.props.onFilterChanged && (this.props.onFilterChanged(property, modifiedFilterValue));
    }
    else{
      this.props.onFilterChanged && (this.props.onFilterChanged(property, filterValue));
    }
  };
  createNumericFilterHandler = (property: string) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const inputValue = event.currentTarget.value.trim();
    console.log(inputValue)
      this.props.onFilterChanged && (this.props.onFilterChanged(property, inputValue));
                ...
                : (col.type === ColumnType.boolean)
                  ...
                  : col.type === ColumnType.text ?(
                    ...
                    <Input
                      ...
                      value={filter[col.property]?.split('*')[0] || ''}
                  ...
                  <Input
                      ...
                      onChange={this.createNumericFilterHandler(col.property)} 
                    />
                )
                    }
            </TableCell>
