...
class MaterialTableComponent<TData extends {} = {}> extends React.Component<MaterialTableComponentProps, MaterialTableComponentState & { contextMenuInfo: { index: number; mouseX?: number; mouseY?: number }; }> {
  ...
  render(): JSX.Element {
    ...
    let linstall:any
    let lactivate:any 
    if (window.location.hash.includes('softwaremanagement')) {
       ldownload = rows.findIndex((row: any) => 
        row.Event === 'Software download' && row.Status === 'COMPLETED' && row.Result === 'SUCCESS'
      );
       linstall = rows.findIndex((row: any) => 
        row.Event === 'Software install' && row.Status === 'COMPLETED' && row.Result === 'SUCCESS'
      );
       lactivate = rows.findIndex((row: any) => 
        row.Event === 'Software active' && row.Status === 'COMPLETED' && row.Result === 'SUCCESS'
      );
      lactivate = rows.findIndex((row: any) => 
        row.Event === 'Software activate' && row.Status === 'COMPLETED' && row.Result === 'SUCCESS'
      );
    ...
    return (
      <Paper className={this.props.className ? `${classes.root} ${this.props.className}` : classes.root} style={{height: this.props?.tableId === "RRH" ? 
      '75%' : window.location.hash.includes("softwaremanagement")? "50%" : "100%" , marginTop: window.location.hash.includes("softwaremanagement") ? ' 15px' :'24px',  borderRadius: window.location.hash.includes("softwaremanagement") ? "10px":'0px',
      boxShadow: window.location.hash.includes("softwaremanagement")  ? "0 4px 8px rgba(0, 0, 0, 0.1)" : '0',
      border: window.location.hash.includes("softwaremanagement") ? "1px solid #ddd" : '0',width:'100%'}}>
        <TableContainer className={classes.container} style={{width:'100%'}}>
          ...
          <Table padding="normal" aria-label={this.props.tableId ? this.props.tableId : 'tableTitle'} stickyHeader={this.props.stickyHeader || false} style={{overflowX: "auto",overflowY:'hidden',width: window.location.hash.includes("fault") ? '150%' :  window.location.hash.includes("systemperformance") ? "120%" : "100%"}} >
            <EnhancedTableHead
              ...
              rows // may need ordering here
                .map((entry: TData & { [RowDisabled]?: boolean, [kex: string]: any }, index) => {
                  ...
                  return (
                    <TableRowExt 
                    style ={{
                      //Adjust this value to decrease row height
                   background: window.location.hash.includes('softwaremanagement') ? index === ldownload || index === linstall || index === lactivate ? 'rgb(0, 153, 51)' : index % 2 ? "#feffff" : "#d1e6f4"  : index % 2 ? "#feffff" : "#d1e6f4"
                    }}
                      hover
                      onClick={event => {
                    ...
                    >
                      ...
                      }
                      {
                        ...
                        this.props.columns.map(
                          col => {
                            const style = col.width ? { width: col.width ,padding: '9px' } : {  padding: '9px' };
                            const tableCell = (
  ...
  private static updateRows(props: MaterialTableComponentPropsWithRows, state: MaterialTableComponentState): { rows: {}[], total: number, page: number } {
    console.log("update enetered")
    let data = [...(props.rows as dataType[] || [])];
    const columns = props.columns;
    console.log("update rows",data)
