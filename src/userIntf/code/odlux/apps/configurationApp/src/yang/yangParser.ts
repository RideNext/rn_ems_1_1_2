...
export class YangParser {
  ...
  public async addCapability(capability: string, version?: string, parentImportOnlyModule?: boolean) {
    ...
    if(localStorage.getItem('yangdataArray') !== null)
    {
      ...
      let yangNodeData = YangData.filter((obj: { nid: string; }) => {
      ...
      let yd=yangNodeData.filter((obj: { capability: string; }) => {
  ...
  {
    data = await yangService.getCapability(capability, this.nodeId, version);
  }
  ...
  public postProcess() {
    ...
    this._groupingsToResolve.filter(vs => vs.uses && vs.uses[ResolveFunction]).forEach(vs => {
      try { vs.uses![ResolveFunction] !== undefined && vs.uses![ResolveFunction]!('|'); } 
      catch (error) {
