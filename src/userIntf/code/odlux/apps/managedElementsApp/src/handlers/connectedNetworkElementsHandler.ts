...
export const {
  actionHandler: testconnectedNetworkElementsActionHandler,
  createActions: testcreateConnectedNetworkElementsActions,
  createProperties: testcreateConnectedNetworkElementsProperties,
  reloadAction: testconnectedNetworkElementsReloadAction,

  // set value action, to change a value
} = createExternal<NetworkElementConnection>(connectedNetworkElementsSearchHandler, appState => appState.managedelements.connectedNetworkElements, 
  (ne) => {
    if (!ne || !ne.id) return true;
    const neUrl = managedElementsRestServices.getNetworkElementUri(ne.id);
