...
class ConnectService {
  ...
  */
  ...
  public getBasicData(path: string) {
    return requestRestExt<{ [key: string]: any }>(path, { method: 'GET' });
  }
  public geCellData(path: string) {
    return requestRestExt<{ [key: string]: any }>(path, { method: 'GET' });
  }

  public getCuupData(path: string) {
    return requestRestExt<{ [key: string]: any }>(path, { method: 'GET' });
  }
  public getCucpData(path: string) {
    return requestRestExt<{ [key: string]: any }>(path, { method: 'GET' });
  }
  
  public getDuData(path: string) {
    return requestRestExt<{ [key: string]: any }>(path, { method: 'GET' });
  }
  
  public getRUData(path: string) {
    return requestRestExt<{ [key: string]: any }>(path, { method: 'GET' });
  }
  ...
  */
  ...
  public async mountNetworkElement(networkElement: NetworkElementConnection): Promise<boolean> {
    ...
    const tlsXml = [
      ...
      ' </protocol>',
      '<max-connection-attempts xmlns="urn:opendaylight:netconf-node-topology">120</max-connection-attempts>',
      '</node>'].join('');
    let bodyXml;
    if (networkElement.password && networkElement.password.trim() !=="") {
      bodyXml = mountXml;
    } else {
      bodyXml = tlsXml;
    }
