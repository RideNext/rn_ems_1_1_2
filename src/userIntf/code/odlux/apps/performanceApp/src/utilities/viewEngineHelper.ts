...
const derivedFrom = (vPath: string, when: WhenAST, viewData: any, includeSelf = false) => {
  ...
  const { performance: { deviceDescription: { modules } } } = storeService.applicationStore.state;
    ...
    if (i < pathParts.length - 1) {
      ...
      if (isViewElementList(viewElement)) {
        for (let j = 0; j < dataUrls.length; ++j) {
          ...
          const restResult = (await performanceRestServices.getConfigData(dataUrl));
