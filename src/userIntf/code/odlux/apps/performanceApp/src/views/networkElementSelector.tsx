...
class NetworkElementSelectorComponent extends React.Component<NetworkElementSelectorComponentProps> {
  ...
  componentDidMount() {
    ...
    if (!initialSorted) {
      initialSorted = true;
      this.props.connectedNetworkElementsActions.onHandleRequestSort('nodeId');
    } else
  ...
  render() {
    return (
      <ConnectedElementTable stickyHeader title={"Performance"} tableId="configurable-elements-table" onHandleClick={(e, row) => { this.props.history.push(`${this.props.match.path}/${row.nodeId}`); }} columns={[
