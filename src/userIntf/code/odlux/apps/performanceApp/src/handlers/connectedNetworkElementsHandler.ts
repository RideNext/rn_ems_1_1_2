...
} = createExternal<NetworkElementConnection>(connectedNetworkElementsSearchHandler, appState => appState.performance.connectedNetworkElements, 
  (ne) => {
    if (!ne || !ne.id) return true;
    const neUrl = performanceRestServices.getNetworkElementUri(ne.id);
