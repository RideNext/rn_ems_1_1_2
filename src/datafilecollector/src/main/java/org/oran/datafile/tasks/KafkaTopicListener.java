...
public class KafkaTopicListener {
    ...
    private Flux<DataFromTopic> startReceiveFromTopic() {
        logger.info("Listening to kafka topic: {}, client id: {}", appConfig.getInputTopic(),
            appConfig.getKafkaClientId());
        return KafkaReceiver.create(kafkaInputProperties()) //
            .receive() //
            .doOnNext(
                input -> logger.info("Received from kafka topic: {} :{}", appConfig.getInputTopic(), input.value())) //
	    ...
	    .doOnNext(data -> 
            logger.info("Mapped DataFromTopic: "+ data)
        ) //
            .publish() //
            .autoConnect();
    }
