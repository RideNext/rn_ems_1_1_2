...
public class FileCollector {
    ...
    private Mono<Optional<FilePublishInformation>> tryCollectFile(FileData fileData) {
        logger.info("starting to collectFile {}", fileData.fileInfo.name);
        ...
        try (FileCollectClient currentClient = createClient(fileData)) {
            ...
            logger.info("isRunning "+ this.appConfig.checkfilestore());
            logger.info("local file "+localFile);
            if(this.appConfig.checkfilestore()){System.out.println("if enetered");Files.createDirectories(localFile.getParent());}
