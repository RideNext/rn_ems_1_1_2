...
public class ORanDOMNetworkElement implements NetworkElement {
    ...
    public void register() {
        ...
        String NodeId = netconfDomAccessor.getNodeId().getValue();
        LOG.info("************************");
        LOG.info("NodeId: {}", NodeId);
        LOG.info("**************************");
        CloseableHttpClient httpClient = HttpClients.createDefault();
        String url = "http://persistence:9200/pre_provider/_doc/" + NodeId;
        ...
        try {
            ...
            if (responseEntity != null) {
                ...
                // Parse the response to extract "DeviceType"
                ObjectMapper objectMapper = new ObjectMapper();
                JsonNode rootNode = objectMapper.readTree(responseBody);

                // Assuming "DeviceType" is a field in the JSON response
                JsonNode deviceTypeNode = rootNode.path("_source").path("DeviceType");
                if (!deviceTypeNode.isMissingNode()) {
                    String deviceType = deviceTypeNode.asText();
                    LOG.info("DeviceType: {}", deviceType);
                    if(deviceType.equalsIgnoreCase("gNodeB")){
                      LOG.info("Device {} is {}. No subscription ",NodeId, deviceType);
                    }
                    else{
                        LOG.info("Device {} is {}. Successfully subscribed ",NodeId, deviceType);
                        netconfDomAccessor.invokeCreateSubscription();
                    }
                } else {
                    LOG.warn("DeviceType field not found in the response.");
                }
            }
        } catch (Exception e) {
