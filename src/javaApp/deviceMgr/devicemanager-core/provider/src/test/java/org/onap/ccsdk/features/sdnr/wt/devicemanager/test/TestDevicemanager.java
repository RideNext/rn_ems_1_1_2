...
public class TestDevicemanager extends Mockito {
    ...
    public void testFaultNotification() {
        ...
        n.faultNotification(faultLogEntityBuilder.setNodeId("node1").setSeverity(SeverityType.Critical)
                .setTimestamp(Instant.parse(new DateAndTime(NetconfTimeStampImpl.getTestpatternDateAndTime()).getValue()).toEpochMilli()*1000).build());
    ...
    public void testPushFault() {
        ...
        inputBuilder.setTimestamp( Instant.parse(new DateAndTime("2020-01-01T01:02:03.4Z").getValue()).toEpochMilli()*1000);
