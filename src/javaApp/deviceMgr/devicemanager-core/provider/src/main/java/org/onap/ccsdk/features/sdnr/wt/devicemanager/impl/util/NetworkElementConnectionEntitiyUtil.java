...
public class NetworkElementConnectionEntitiyUtil {
     ...
     */
    public static NetworkElementConnectionEntity getNetworkConnectionDeviceDetails(String deviceType,String modelNumber,String softwareVersion ,String serialNumber ,String vendorDetails, String HeartBeat) {
        NetworkElementConnectionBuilder eb = new NetworkElementConnectionBuilder();
        eb.setDeviceType(deviceType);

        eb.setModelNumber(modelNumber);
        eb.setSoftwareVersion(softwareVersion);
        eb.setSerialNumber(serialNumber);
        eb.setVendorDetails(vendorDetails);
        eb.setHeartBeatStatus(HeartBeat);
     ...
     * Update devicetype and let all other field emptys
     *
     * @param isStartUpdone that should be updated
     * @return NetworkElementConnectionEntity with related parameter
     */
    public static NetworkElementConnectionEntity startUpXMLStatusUpdate(String isStartUpdone) {
        NetworkElementConnectionBuilder eb = new NetworkElementConnectionBuilder();
        eb.setIsStartUpdone(isStartUpdone);
        return eb.build();
    }
     ...
     */
    public static NetworkElementConnectionEntity getNetworkConnection(String nodeId, @Nonnull NetconfNode nNode, String deviceType) {
        return getNetworkConnection(nodeId, nNode, Optional.empty(),deviceType);
    }

    public static NetworkElementConnectionEntity getNetworkConnection(String nodeId, @Nonnull NetconfNode nNode,
            Optional<NetconfNode> configNetconfNode,String deviceType) {

        NetworkElementConnectionBuilder eb = new NetworkElementConnectionBuilder();
        // -- basics
        eb.setId(nodeId).setNodeId(nodeId).setDeviceType(deviceType).setIsRequired(true);
