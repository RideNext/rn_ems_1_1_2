...
public class DataObjectAcessorStatus extends DataObjectAcessor<Data> {
    private static final Logger LOG = LoggerFactory.getLogger(DataObjectAcessorStatus .class);
    final String ESDATATYPE_FAULTCURRENT_SEVERITY_KEY = "event.fault-fields.event-severity";
    ...
    public QueryResult<Data> getDataStatus(EntityInput input) throws IOException {
        ...
        response  = this.dbClient.search(request);
            LOG.info("faulttttt response {}",response);
       
        
        aggs = response.getAggregations(ESDATATYPE_FAULTCURRENT_SEVERITY_KEY);
       LOG.info("faulttttt data {}",aggs);
        }
        catch(Exception e){
            query.aggregations(ESDATATYPE_FAULTCURRENT_SEVERITY_KEY+".keyword").size(0);
        if(this.doFullsizeRequest) {
            query.doFullsizeRequest();
        }
        request.setQuery(query);
        ...
        aggs= response.getAggregations(ESDATATYPE_FAULTCURRENT_SEVERITY_KEY+".keyword");
       LOG.info("faulttttt data {}",aggs);
        }
        ...
        aggs = response.getAggregations(ESDATATYPE_NECON_CONNECTIONSTATE_KEY);
            ...
            query.aggregations(ESDATATYPE_NECON_CONNECTIONSTATE_KEY+".keyword").size(0);
        if(this.doFullsizeRequest) {
            query.doFullsizeRequest();
        }
        request.setQuery(query);
        response = this.dbClient.search(request);
        aggs = response.getAggregations(ESDATATYPE_NECON_CONNECTIONSTATE_KEY+".keyword");
        }
