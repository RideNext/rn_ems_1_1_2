...
public class TestMapper {
    ...
    public void testYangGenEnumMapperSer() {
        NetworkElementConnection con =
                new NetworkElementConnectionBuilder().setDeviceType("gNodeB").build();
        String str = null;
        try {
        ...
        assertEquals("O-RAN", new JSONObject(str).getString("gNodeB"));
        con = new NetworkElementConnectionBuilder().setDeviceType("gNodeB").build();
        str = null;
        try {
            str = MAPPER.writeValueAsString(con);
        } catch (JsonProcessingException e) {
