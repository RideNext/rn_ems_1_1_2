...
public class JsonNormalizedNodeBodyWriter extends AbstractNormalizedNodeBodyWriter {
    private static final Logger LOG = LoggerFactory.getLogger(JsonNormalizedNodeBodyWriter.class);
    ...
    private static void writeNormalizedNode(final JsonWriter jsonWriter, final InstanceIdentifierContext context,
        ...
        if (schemaNode instanceof RpcDefinition rpc) {
            ...
            // RpcDefinition is not supported as initial codec in JSONStreamWriter, so we
            // need to emit initial output
            // declaration
            nnWriter = createNormalizedNodeWriter(
                    ...
                    fields);
            try {
                final Module module = context.getSchemaContext()
                        .findModule(data.getIdentifier().getNodeType().getModule())
                        .orElseThrow();
                ...
                LOG.error("Exception {}", ex2.getStackTrace(), ex2);
        // jsonWriter.name("ietf-netconf" + ":output");
        // jsonWriter.beginObject();
        // writeChildren(nnWriter, (ContainerNode) data);
        // jsonWriter.endObject();
            }
        } else if (schemaNode instanceof ActionDefinition action) {
            // ActionDefinition is not supported as initial codec in JSONStreamWriter, so we
            // need to emit initial output
            // declaration
    ...
    private static XMLNamespace initialNamespaceFor(final SchemaNode schema) {
        ...
        // For top-level elements we always want to use namespace prefix, hence use a
        // null initial namespace
        return null;
    }
