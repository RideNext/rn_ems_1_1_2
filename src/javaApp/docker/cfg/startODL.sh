...
initialize_sdnrdb() {
  ...
  bash /opt/sdnc/bin/createIndex.sh
  return $?
}
...
if $JDEBUG ; then
    ...
    EXTRA_JAVA_OPTS="${EXTRA_JAVA_OPTS} -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=0.0.0.0:1090 -Djavax.net.debug=ssl,handshake"
