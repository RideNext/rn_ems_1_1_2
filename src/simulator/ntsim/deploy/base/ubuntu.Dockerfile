...
RUN \
    ...
    git clone --single-branch --branch master  https://github.com/libssh2/libssh2.git
...
RUN \
    cd libyang && \
    mkdir build && cd build  && \
    cmake -DCMAKE_BUILD_TYPE:String="Debug" -DGEN_LANGUAGE_BINDINGS=ON -DGEN_CPP_BINDINGS=ON -DGEN_PYTHON_BINDINGS=OFF -DENABLE_BUILD_TESTS=OFF .. && \
    make -j4  && \
    make install && \
    ldconfig
...
RUN \
    cd netopeer2 && \
    mkdir build && cd build && \
    cmake -DCMAKE_BUILD_TYPE:String="Debug" -DGENERATE_HOSTKEY=OFF -DMERGE_LISTEN_CONFIG=OFF .. && \
    make -j4 && \
    make install
