...
static void* ves_o1_pnf_registration_thread_routine(void *arg) {
   ...
   log_add_verbose(2, "RN-EMS: Sending: THE O1 PNF registration (stndDefined) \n");
   //sleep(10);
   ...
   if ( current_session  ) {
	  ...
	  int is_lock = 0;
	  uint32_t l_id = 0 , id = 0;
	  time_t timestamp;

          log_add_verbose(2, "RN-EMS: : GET LOCK FROM CURRENT SESSION: CANDIDATE \n");

	  sr_conn_ctx_t *conn  = sr_session_get_connection( current_session );
	  // Check if the DB locked 
	  int lck = sr_get_lock( conn , SR_DS_CANDIDATE , NULL ,  &is_lock , &l_id , &id , &timestamp);
	  if ( lck != SR_ERR_OK ) {
              log_add_verbose(2, "********* RN-EMS: : FAILURE TO GET LOCK : ***************  \n");
              log_error( "********* RN-EMS: : FAILURE TO GET DB  LOCK : ***************  \n");
	    }  
	  else {
                  log_add_verbose(2, "RN-EMS: : GOT LOCK : CHECK STATUS : CANDIDATE [%s]  \n", ( (is_lock) ? "YES" : "NO" ) );
		  if ( is_lock ) {
                      log_add_verbose(2, "**** RN-EMS: : TRYING TO  UN LOCK :   *****  \n" );
		      lck = sr_unlock( current_session , NULL) ;
	               if ( lck != SR_ERR_OK ) {
                            log_add_verbose(2, "********* RN-EMS: : FAILURE TO UN LOCK : [%d] ***************  \n",lck);
                            log_error( "********* RN-EMS: : FAILURE TO UN LOCK : [%d]***************  \n",lck);
	                  }  
		        else {
                            log_add_verbose(2, "********* RN-EMS: : SUCCESSFULL TO UN LOCK : [%d] ***************  \n",lck);
                            log_error( "********* RN-EMS: : SUCCESSFULL UN LOCK : [%d]***************  \n",lck);
			}
    ...
    while(remaining) {
	   ...
	   if ( total_regs > MAX_CNT ) {
                // If total number of registration more than 59; send periodically for
                // rather than contniuosly
                if( ++initCnt > MAX_CNT ) {
                    initCnt = 0;
                    log_add_verbose(2, " ===== PNF-O1-VES REGISTRATION SENT TILL COUNT[%d]: PORT[%d] ==== WAIT 10 SEC ==== \n", i, regs[i].port);
                    sleep(10);
                 }
            }
