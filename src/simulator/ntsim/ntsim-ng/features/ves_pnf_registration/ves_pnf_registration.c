...
static void* ves_pnf_registration_thread_routine(void *arg) {
    ...
    log_add_verbose(2, "PNF registration Sending:\n");
    //sleep(10);
    ...
    while(remaining) {
	    ...
	    if ( total_regs > MAX_CNT ) {
                // If total number of registration more than MAX_CNT; send periodically for
                // rather than contniuosly
                if( ++initCnt > MAX_CNT ) {
                    initCnt = 0;
                    log_add_verbose(2, " ===== PNF-O1 REGISTRATION SENT TILL COUNT[%d]: PORT[%d] ==== WAIT 10 SEC ==== \n", i, regs[i].port);
                    sleep(10);
                 }
            }
        }
...
static cJSON* ves_create_pnf_registration_fields(const char *nf_ip_v4_address, const char *nf_ip_v6_address, int nf_port, bool is_tls) {
    ...
    if(cJSON_AddStringToObject(pnf_registration_fields, "modelNumber", "RN4XCJ2023") == 0) {
        log_error("cJSON_AddItemToObject failed\n");
        cJSON_Delete(pnf_registration_fields);
        return 0;
    }
    ...
    sprintf(serial_number, "%s-%s", framework_environment.settings.hostname, "RNCX05");
    ...
    if(cJSON_AddStringToObject(pnf_registration_fields, "vendorName", "RIDENEXT") == 0) {
        log_error("cJSON_AddItemToObject failed\n");
        cJSON_Delete(pnf_registration_fields);
        return 0;
    }
