...
uint32_t g_alarmId[36] = {'\0'};

static cJSON *ves_create_fault_fields(const char *alarm_condition, const char *alarm_object, const char *severity, const char *date_time, const char *specific_problem, const char* alarmId);
static int ves_message_send_internal(sr_session_ctx_t *session, const char *condition, const char *object, const char *severity, const char *date_time, const char *specific_problem, int port, uint32_t *seq_id, const char *alarmId);
...
int faults_ves_message_send(sr_session_ctx_t *session, const char *condition, const char *object, const char *severity, const char *date_time, const char *specific_problem, const char* alarmId) {
    assert(condition);
    assert(object);
    assert(severity);
    assert(date_time);
    ...
    if ( alarmId )
       strcpy(g_alarmId,alarmId);  
    else
       strcpy(g_alarmId,"RNALARMID-33"); //HARDCODED IF NO ALAMID FOUND  

    log_error("RN-EMS: COPYING ALARMID [%s]\n", g_alarmId);
    ...
    if((framework_environment.settings.ssh_connections + framework_environment.settings.tls_connections) > 1) {
        for(int port = ssh_base_port; port < ssh_base_port + framework_environment.settings.ssh_connections; port++) {
            ...
            int rc = ves_message_send_internal(session, condition, object, severity, date_time, specific_problem, port, seq_id, alarmId);
            if(rc != NTS_ERR_OK) {
        ...
        uint32_t *seq_id = &fault_ves_sequence_number[sequence_index++];
        int rc = ves_message_send_internal(session, condition, object, severity, date_time, specific_problem, 0, seq_id,alarmId);
        if(rc != NTS_ERR_OK) {
            log_error("ves_message_send_internal failed\n");
        }
    }
...
static cJSON *ves_create_fault_fields(const char *alarm_condition, const char *alarm_object, const char *severity, const char *date_time, const char *specific_problem, const char* alarmId) {
    ...
    if( strcmp(severity,"NORMAL") == 0 || strcmp(severity,"normal") == 0 ) {
          strcpy(alarmAction, "CLEAR");
      } 
    else{

       strcpy(alarmAction, "RAISE");
     }

    if ( alarmId ) {

        log_error("RN-EMS: UPDATING ALARM ID[%s] AND TYPE INFO:[%s] \n",alarmId,alarmAction);
        if(cJSON_AddStringToObject(alarmAdditionalInformation, "alarmId", alarmId) == 0) {
        ...
        log_error("RN-EMS: UPDATING ALARM ID[%s] AND TYPE INFO:[%s] \n","NULL",alarmAction);
        if(cJSON_AddStringToObject(alarmAdditionalInformation, "alarmId", "TEJAS_RN_ALARMID_33") == 0) {
            log_error("could not create JSON object: eventTime\n");
            cJSON_Delete(faultFields);
            return 0;
         }
    ...
    if(cJSON_AddStringToObject(alarmAdditionalInformation, "AlarmAction", alarmAction) == 0) {
        log_error("could not create JSON object: eventSeverity\n");
        cJSON_Delete(faultFields);
        return 0;
    }
...
static int ves_message_send_internal(sr_session_ctx_t *session, const char *condition, const char *object, const char *severity, const char *date_time, const char *specific_problem, int port, uint32_t *seq_id, const char* alarmId) {
    ...
    cJSON *fault_fields = ves_create_fault_fields(condition, object, severity, date_time, specific_problem, alarmId);
    if(fault_fields == 0) {
