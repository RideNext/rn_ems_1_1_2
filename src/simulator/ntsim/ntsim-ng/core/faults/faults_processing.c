...
fault_settings_t *faults_settings_read(const char *json_plain) {
    ...
    if(node && cJSON_IsArray(node)) {
        ...
        cJSON_ArrayForEach(fault_detail, node) {
            ...
            ret->fault[ret->fault_count - 1].alarmId = 0;
...
int faults_settings_process(fault_settings_t *faults, int fault_no) {
    log_error(" RN-EMS: faults_settings_process: [%d] \n", fault_no);
    assert(faults);
    ...
    if( faults->fault[fault_no].alarmId )
       free(faults->fault[fault_no].alarmId);
    ...
    faults->fault[fault_no].alarmId = 0;
    ...
    for(int j = 0; j < faults->fault[fault_no].field_count; j++) {
        ...
        else if(strcmp(faults->fault[fault_no].field_name[j], "%%alarmId%%") == 0) {
            faults->fault[fault_no].alarmId = fault_process_vars(faults->fault[fault_no].field_value[j], &faults->fault[fault_no]);
        }
    }
    ...
    if(faults->fault[fault_no].alarmId == 0) {
        log_error("RN_SIM: could not find alarmId in fault\n");
        return NTS_ERR_FAILED;
    }
