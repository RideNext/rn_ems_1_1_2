...
#CURRDIR=${PWD}
#ENV_PATH=${CURRDIR}/../../exec/smo/oam/.env
#source ${ENV_PATH}
...
EOF
    ...
    if [[ "$(echo "${existing_doc}" | jq -r '.found')" == "true" ]]; then
        curl -s -X POST "${ES_URL}/${INDEX_NAME}/_update/${doc_id}" \
    -H 'Content-Type: application/json' \
    -d "{
          \"script\": {
            \"source\": \"if (ctx._source.data == null) { ctx._source.data = params.new_data; } else { ctx._source.data.addAll(params.new_data); } ctx._source.status = params.status;\",
            \"params\": {
              \"new_data\": [${DATA}],
              \"status\": \"${status}\"
            }
